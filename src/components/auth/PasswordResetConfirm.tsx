...
// ⭐️ Import the core confirmResetPassword function
import { confirmResetPassword } from 'aws-amplify/auth';

// ⭐️ Accept the username as a prop from AuthLayout
interface PasswordResetConfirmProps {
  username: string;
}

export function PasswordResetConfirm({ username }: PasswordResetConfirmProps) {
  const { toSignIn, toForgotPassword } = useAuthenticator((context) => [
    context.toSignIn,
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Call the core function with all required parameters
      await confirmResetPassword({
        username,
        confirmationCode: code,
        newPassword,
      });
      // On success, the route automatically changes back to 'signIn'
            ...
            <div className="px-6 pt-4 pb-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <button type="button" onClick={toForgotPassword} className="text-sm text-white/70 hover:text-white hover:underline underline-offset-4 flex items-center gap-1.5">
                    <ArrowLeft className="w-4 h-4" />
                    Back
                  </button>
                  <button type="button" onClick={toSignIn} className="text-sm text-white/70 hover:text-white hover:underline underline-offset-4">
                    Back to sign in
                  </button>
                </div>
              </form>
