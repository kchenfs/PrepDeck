...
export function IntegrationsPage() {
  ...
  // --- Effects ---
  useEffect(() => {
    // ... (This useEffect hook is perfect as-is)
  ...
  const handleUberConnect = async () => {
    ...
    let userId;
    try {
      // --- 3. GET CURRENT USER'S ID (this is the Cognito 'sub') ---
      const { userId: cognitoSub } = await getCurrentUser();
      if (!cognitoSub) {
        throw new Error("User ID (sub) not found.");
      }
      userId = cognitoSub;
      console.log("Using user ID for state parameter:", userId);

    } catch (error) {
      console.error("Could not get authenticated user:", error);
      alert("You must be logged in to connect an integration. Please sign in and try again.");
    ...
    // --- 4. ADD THE USER ID AS THE 'state' PARAMETER ---
    const state = userId;
    const authorizeUrl = `https://auth.uber.com/oauth/v2/authorize?client_id=${clientId}&response_type=code&scope=${scope}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${encodeURIComponent(state)}`;
  ...
  // --- Component Return ---
  return (
    // ... (The rest of your component's JSX is perfect as-is)
